...
public class DescTableCommand implements Command {

    private final Object tableName;

    public DescTableCommand(Object tableName) {
    ...
    public void execute() {
        try {
            List<Map<String, Object>> rows  = DbAccessor.getInstance().describeTable(String.valueOf(tableName), JdbcUtils::extractToMap);
            if (rows.size() > 0) {
